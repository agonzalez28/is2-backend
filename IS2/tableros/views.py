...
from .models import tableros, ListasTableros
...
def agregar_lista(request):
    if request.method == "POST":
        try:
            ...
            cant_tarjetas = data.get("cant_tarjetas", 1)
            cod_tablero = 1#data.get("cod_tablero")

            # Validación
            if nom_lista is None:
                return JsonResponse({"error": "El nombre de la lista es obligatorio"}, status=400)

            # Buscar el objeto de tablero a partir del ID de `cod_tablero`
            try:
                tablero_obj = tableros.objects.get(cod_tablero=cod_tablero)
            except tableros.DoesNotExist:
                return JsonResponse({"error": "Tablero no encontrado"}, status=404)

            # Crear la nueva lista usando el objeto de tablero
            nueva_lista = ListasTableros.objects.create(
                nom_lista=nom_lista,
                cant_tarjetas=cant_tarjetas,
                cod_tablero=tablero_obj  # Pasar el objeto en vez del ID
            )

            return JsonResponse({"mensaje": "Lista agregada exitosamente", "id": nueva_lista.cod_lista}, status=201)
